...
function taskRow(tr,day,t){const done=isDone(tr,day,t.id),r=document.createElement('div');r.className='task tracker-action-row'+(done?' done':'');r.innerHTML=`<div class="grow"><div class="task-label">${esc(t.label)}</div>${t.description?`<div class="task-description">${esc(t.description)}</div>`:''}<div class="task-meta">${t.periodLabel?esc(t.periodLabel)+' · ':''}${t.time||''}${t.notify?' · 🔔':''}</div></div><div class="today-choice-group"><button class="history-choice done-choice ${done?'selected':''}" aria-pressed="${done?'true':'false'}">✓ בוצע</button></div>`;r.querySelector('.done-choice').onclick=()=>toggleDone(tr,day,t.id);return r}
...
function openHistoryEditor(tr,day){activeHistoryDay=day;const bg=document.getElementById('historyModalBackdrop');if(!bg)return;renderHistoryEditorModal(tr,day);bg.classList.remove('hidden')}
