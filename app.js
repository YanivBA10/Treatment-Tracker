...
function renderHistoryEditorModal(tr,day){const title=document.getElementById('historyModalTitle'),dateEl=document.getElementById('historyModalDate'),body=document.getElementById('historyModalTasks');if(!title||!dateEl||!body)return;const date=parseDate(tr.startDate);date.setDate(date.getDate()+day-1);title.textContent=`עדכון יום ${day}`;dateEl.textContent=fmtDate(date);body.innerHTML='';const ts=tasksForDay(tr,day);if(!ts.length){body.innerHTML='<div class="empty">לא היו פעולות ביום הזה.</div>';return}for(const t of ts){const done=isDone(tr,day,t.id),r=document.createElement('div');r.className='task'+(done?' done':'');r.innerHTML=`<div class="grow"><div class="task-label">${esc(t.label)}</div>${t.description?`<div class="task-description">${esc(t.description)}</div>`:''}<div class="task-meta">${t.periodLabel?esc(t.periodLabel)+' · ':''}${t.time||''}${t.notify?' · 🔔':''}</div></div><button class="check">${done?'✓':''}</button>`;r.querySelector('button').onclick=()=>{tr.done ||= {};tr.done[dayKey(day)] ||= {};tr.done[dayKey(day)][t.id]=!tr.done[dayKey(day)][t.id];save();renderHistoryEditorModal(tr,day);renderDetailHistory(tr);renderMain()};body.appendChild(r)}}
