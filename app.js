...
function loadState(){
  ...
  if(existing){existing.version=6;existing.settings||={theme:'system'};if(!existing.settings.theme)existing.settings.theme='system';existing.trackers||=[];existing.reminders||=[];existing.tasks||=[];for(const wt of existing.tasks)if(wt.showOnMain==null)wt.showOnMain=false;for(const tr of existing.trackers){tr.skipped||={};for(const s of (tr.stages||[]))for(const t of (s.tasks||[]))if(t.description==null)t.description='';}return existing;}
  const old=JSON.parse(localStorage.getItem(OLD_KEY)||'null');
  const base={version:6,trackers:[],reminders:[],tasks:[],settings:{theme:'system'}};
  if(old){base.trackers.push(migrateOld(old));localStorage.setItem(APP_KEY,JSON.stringify(base));}
  return base;
}
function migrateOld(old){
  return {id:uid('tracker'),name:'טיפול משחה ואבקה',startDate:old.startDate||todayStr(),duration:30,openEnded:false,mode:'staged',status:'active',createdAt:Date.now(),done:old.done||{},skipped:{},stages:[
...
function isSkipped(tr,day,id){return !!(tr.skipped&&tr.skipped[dayKey(day)]&&tr.skipped[dayKey(day)][id])}
function toggleDone(tr,day,id){tr.done ||= {};tr.done[dayKey(day)] ||= {};tr.skipped ||= {};tr.skipped[dayKey(day)] ||= {};const next=!tr.done[dayKey(day)][id];tr.done[dayKey(day)][id]=next;if(next)tr.skipped[dayKey(day)][id]=false;save();renderDetail();renderMain();}
...
function renderMain(){
  ...
  if(act){act.innerHTML='';const active=state.trackers.filter(t=>t.status==='active');if(!active.length)act.innerHTML='<div class="empty">אין כרגע מעקבים פעילים.</div>';else active.forEach(t=>act.appendChild(trackerCard(t)));}
  if(arc){arc.innerHTML='';const archived=state.trackers.filter(t=>t.status==='archived');if(!archived.length)arc.innerHTML='<div class="empty">אין מעקבים בארכיון.</div>';else archived.forEach(t=>arc.appendChild(trackerCard(t,true)));}
  if(rem){rem.innerHTML='';const activeR=state.reminders.filter(r=>r.status!=='completed').sort((a,b)=>reminderSortKey(a).localeCompare(reminderSortKey(b)));if(!activeR.length)rem.innerHTML='<div class="empty compact">אין כרגע תזכורות פעילות.</div>';else activeR.forEach(r=>rem.appendChild(reminderCard(r,false)));}
  if(doneRem){doneRem.innerHTML='';const completedR=state.reminders.filter(r=>r.status==='completed').sort((a,b)=>(b.completedAt||0)-(a.completedAt||0));if(!completedR.length)doneRem.innerHTML='<div class="empty compact">אין תזכורות שהושלמו.</div>';else completedR.forEach(r=>doneRem.appendChild(reminderCard(r,true)));}
  renderTasksList();renderTodayDashboard();renderNotificationStatus();
}
function trackerCard(tr,archived=false){const el=document.createElement('div');el.className='tracker';const st=todayStats(tr),all=totalStats(tr),completedToday=st.day>=1&&st.total>0&&st.done===st.total;let todayText=completedToday?'הושלם להיום ✓':(st.day>=1&&st.total?`${st.done}/${st.total} היום`:(st.day<1?'טרם התחיל':'אין פעולות היום'));
  el.innerHTML=`<div class="tracker-top"><div><div class="tracker-name">${esc(tr.name)}</div><div class="small">${trackerLabel(tr)} · ${todayText}</div></div><div class="badge ${completedToday?'ok':''}">${archived?'בארכיון':(completedToday?'✓ היום':all.pct+'%')}</div></div><div class="progress"><div style="width:${all.pct}%"></div></div>${archived?'<div class="actions"><button class="btn ghost smallbtn" data-restore>החזר לפעילים</button></div>':''}`;
...
function renderTrackerRoute(id,section='today'){const tr=state.trackers.find(t=>t.id===id);if(!tr){replaceRoute({route:'main',view:'trackersView'});return}currentTrackerId=id;editingTrackerId=null;detailSection=section||'today';showOnly('detailView');document.getElementById('bottomTabs').classList.add('hidden');renderDetail();}
function currentTracker(){return state.trackers.find(t=>t.id===currentTrackerId)}
function renderDetail(){const tr=currentTracker();if(!tr)return;document.getElementById('detailName').textContent=tr.name;document.getElementById('detailMeta').textContent=`${trackerLabel(tr)} · התחלה ${fmtDate(parseDate(tr.startDate),true)}`;const today=todayStats(tr),completedToday=today.day>=1&&today.total>0&&today.done===today.total;const detailBadge=document.getElementById('detailBadge');detailBadge.textContent=tr.status==='archived'?'בארכיון':(completedToday?'הושלם להיום ✓':'פעיל');detailBadge.classList.toggle('ok',tr.status!=='archived'&&completedToday);const all=totalStats(tr);document.getElementById('detailProgress').style.width=all.pct+'%';document.querySelectorAll('.detail-nav button').forEach(b=>b.classList.toggle('active',b.dataset.detail===detailSection));document.getElementById('detailToday').classList.toggle('hidden',detailSection!=='today');document.getElementById('detailHistory').classList.toggle('hidden',detailSection!=='history');document.getElementById('detailSettings').classList.toggle('hidden',detailSection!=='settings');renderDetailToday(tr);renderDetailHistory(tr);renderDetailSettings(tr);}
function renderDetailToday(tr){const box=document.getElementById('detailToday'),day=trackerDay(tr);box.innerHTML='';const top=document.createElement('div');top.className='card';top.innerHTML=`<div class="section-title">${day<1?'המעקב עדיין לא התחיל':(!tr.openEnded&&day>tr.duration?'המעקב הסתיים':`היום · יום ${day}`)}</div><div class="small">${fmtDate(new Date())}</div>`;box.appendChild(top);const tasks=tasksForDay(tr,day);const c=document.createElement('div');c.className='card';c.innerHTML='<div class="section-title">הפעולות להיום</div>';if(!tasks.length)c.innerHTML+='<div class="empty">אין פעולות מתוכננות להיום.</div>';tasks.forEach(t=>c.appendChild(taskRow(tr,day,t)));if(tasks.length){const done=tasks.filter(t=>isDone(tr,day,t.id)).length;const s=document.createElement('div');s.className='summary'+(done===tasks.length?' completed-today':'');s.textContent=done===tasks.length?'הושלם להיום ✓':`${done} מתוך ${tasks.length} הושלמו היום`;c.appendChild(s)}box.appendChild(c);
  const pending=[];if(tr.status!=='archived')for(let d=1;d<Math.min(day,trackerEndDay(tr)+1);d++){const ts=tasksForDay(tr,d);if(ts.length&&ts.some(t=>!isDone(tr,d,t.id)&&!isSkipped(tr,d,t.id)))pending.push(d)}if(pending.length){const n=document.createElement('div');n.className='card notice';n.innerHTML=`<div class="section-title">יש פעולות קודמות שדורשות עדכון</div><div class="summary">${pending.length===1?`ביום ${pending[0]} יש פעולה שעדיין לא סומנה כבוצעה או כלא בוצעה.`:`ב־${pending.length} ימים קודמים יש פעולות שעדיין לא טופלו.`}</div><button class="btn secondary block">טפל בפעולות שלא עודכנו</button>`;n.querySelector('button').onclick=()=>{detailSection='history';renderDetail();setTimeout(()=>openHistoryEditor(tr,pending[0]),20)};box.appendChild(n)}
}
function taskRow(tr,day,t){const done=isDone(tr,day,t.id),archived=tr.status==='archived',r=document.createElement('div');r.className='task tracker-action-row'+(done?' done':'')+(archived?' tracker-action-readonly':'');r.innerHTML=`<div class="grow"><div class="task-label">${esc(t.label)}</div>${t.description?`<div class="task-description">${esc(t.description)}</div>`:''}<div class="task-meta">${t.periodLabel?esc(t.periodLabel)+' · ':''}${t.time||''}${t.notify?' · 🔔':''}</div></div><div class="today-choice-group"><button class="history-choice done-choice ${done?'selected':''}" aria-pressed="${done?'true':'false'}" aria-label="${done?'בוצע':'סמן כבוצע'}" ${archived?'disabled aria-disabled="true" title="החזר לפעילים כדי לעדכן"':''}>${done?'✓ בוצע':'סמן כבוצע'}</button></div>`;if(!archived)r.querySelector('.done-choice').onclick=()=>toggleDone(tr,day,t.id);return r}
function renderDetailHistory(tr){const box=document.getElementById('detailHistory');box.innerHTML='<div class="card"><div class="section-title">היסטוריה</div><div class="small history-hint">לחץ על יום כדי לעדכן את הפעולות שלו.</div><div id="historyRows"></div></div>';const rows=box.querySelector('#historyRows'),today=trackerDay(tr),max=tr.openEnded?Math.max(30,today):tr.duration;for(let d=1;d<=max;d++){const ts=tasksForDay(tr,d);const done=ts.filter(t=>isDone(tr,d,t.id)).length,date=parseDate(tr.startDate);date.setDate(date.getDate()+d-1);const row=document.createElement('div');row.className='history-day'+(d>today?' future':'');let txt=d===today?`היום · ${done}/${ts.length}`:`${done}/${ts.length}`,cls='';if(d<today&&ts.length){if(done===ts.length){txt='הושלם';cls='ok'}else cls='miss'}row.innerHTML=`<div><strong>יום ${d}</strong><div class="task-meta">${fmtDate(date,true)}</div></div><div class="status ${cls}">${txt}</div>`;if(d<=today)row.onclick=()=>openHistoryEditor(tr,d);rows.appendChild(row)}}
function openHistoryEditor(tr,day){if(tr.status==='archived'){toast('המעקב בארכיון. החזר לפעילים כדי לעדכן.');return}activeHistoryDay=day;const bg=document.getElementById('historyModalBackdrop');if(!bg)return;renderHistoryEditorModal(tr,day);bg.classList.remove('hidden')}
function renderHistoryEditorModal(tr,day){const title=document.getElementById('historyModalTitle'),dateEl=document.getElementById('historyModalDate'),body=document.getElementById('historyModalTasks');if(!title||!dateEl||!body)return;const date=parseDate(tr.startDate);date.setDate(date.getDate()+day-1);title.textContent=`עדכון יום ${day}`;dateEl.textContent=fmtDate(date);body.innerHTML='';const ts=tasksForDay(tr,day);if(!ts.length){body.innerHTML='<div class="empty">לא היו פעולות ביום הזה.</div>';return}const pastDay=day<trackerDay(tr);for(const t of ts){const done=isDone(tr,day,t.id),skipped=isSkipped(tr,day,t.id),r=document.createElement('div');r.className='task history-task'+(done?' done':'')+(skipped?' skipped':'');const actions=pastDay?`<div class="history-choice-group" role="group" aria-label="סטטוס הפעולה"><button class="history-choice done-choice ${done?'selected':''}" aria-pressed="${done?'true':'false'}">✓ בוצע</button><button class="history-choice skipped-choice ${skipped?'selected':''}" aria-pressed="${skipped?'true':'false'}">✕ לא בוצע</button></div>`:`<div class="today-choice-group"><button class="history-choice done-choice ${done?'selected':''}" aria-pressed="${done?'true':'false'}">✓ בוצע</button></div>`;r.innerHTML=`<div class="grow"><div class="task-label">${esc(t.label)}</div>${t.description?`<div class="task-description">${esc(t.description)}</div>`:''}<div class="task-meta">${t.periodLabel?esc(t.periodLabel)+' · ':''}${t.time||''}${t.notify?' · 🔔':''}</div></div>${actions}`;if(pastDay){const doneBtn=r.querySelector('.done-choice'),skipBtn=r.querySelector('.skipped-choice');doneBtn.onclick=()=>{tr.done ||= {};tr.done[dayKey(day)] ||= {};tr.skipped ||= {};tr.skipped[dayKey(day)] ||= {};const next=!tr.done[dayKey(day)][t.id];tr.done[dayKey(day)][t.id]=next;if(next)tr.skipped[dayKey(day)][t.id]=false;save();renderHistoryEditorModal(tr,day);renderDetailHistory(tr);renderDetailToday(tr);renderMain()};skipBtn.onclick=()=>{tr.done ||= {};tr.done[dayKey(day)] ||= {};tr.skipped ||= {};tr.skipped[dayKey(day)] ||= {};const next=!tr.skipped[dayKey(day)][t.id];tr.skipped[dayKey(day)][t.id]=next;if(next)tr.done[dayKey(day)][t.id]=false;save();renderHistoryEditorModal(tr,day);renderDetailHistory(tr);renderDetailToday(tr);renderMain()}}else{r.querySelector('.done-choice').onclick=()=>{tr.done ||= {};tr.done[dayKey(day)] ||= {};tr.skipped ||= {};tr.skipped[dayKey(day)] ||= {};const next=!tr.done[dayKey(day)][t.id];tr.done[dayKey(day)][t.id]=next;if(next)tr.skipped[dayKey(day)][t.id]=false;save();renderHistoryEditorModal(tr,day);renderDetailHistory(tr);renderMain()}}body.appendChild(r)}}
function closeHistoryEditor(){activeHistoryDay=null;document.getElementById('historyModalBackdrop')?.classList.add('hidden')}
function renderDetailSettings(tr){
  const box=document.getElementById('detailSettings');
  const archived=tr.status==='archived';
  box.innerHTML=`<div class="card"><div class="section-title">ניהול המעקב</div>${archived?'<div class="small tracker-archive-readonly-note">המעקב בארכיון ונשמר לקריאה בלבד. החזר אותו לפעילים כדי לערוך את התכנית.</div>':''}<div class="actions">${archived?'':'<button class="btn secondary" data-edit>ערוך</button>'}<button class="btn ghost" data-dup>שכפל</button><button class="btn ghost" data-archive>${archived?'החזר לפעילים':'העבר לארכיון'}</button><button class="btn danger" data-delete>מחק</button></div></div><div class="card"><div class="section-title">תכנית</div><div class="small">${describeTracker(tr)}</div></div>`;
  const edit=box.querySelector('[data-edit]');
  if(edit)edit.onclick=()=>openEditor(tr.id);
  box.querySelector('[data-dup]').onclick=()=>duplicateTracker(tr);
  box.querySelector('[data-archive]').onclick=()=>{tr.status=tr.status==='archived'?'active':'archived';save();renderMain();renderDetail();toast(tr.status==='archived'?'הועבר לארכיון':'הוחזר לפעילים')};
  box.querySelector('[data-delete]').onclick=async()=>{const ok=await showConfirm({title:'למחוק את המעקב?',message:`"${tr.name}" יימחק יחד עם כל ההיסטוריה שלו. לא ניתן לשחזר את הנתונים לאחר המחיקה.`,confirmText:'מחק לצמיתות',danger:true});if(ok){const wasArchived=tr.status==='archived';state.trackers=state.trackers.filter(x=>x.id!==tr.id);save();replaceRoute({route:'main',view:wasArchived?'archiveView':'trackersView'});toast('המעקב נמחק')}};
}
...
function taskDueState(t){
  ...
  const today=todayStr();if(t.dueDate<today)return 'overdue';if(t.dueDate===today)return 'today';return 'future';
}
...
function renderTasksList(){
  ...
  const active=(state.tasks||[]).filter(t=>t.status!=='completed').sort((a,b)=>(a.dueDate||'9999-12-31').localeCompare(b.dueDate||'9999-12-31'));
...
function taskEditor(t,onDelete){
  ...
  w.innerHTML=`<div class="row"><div class="grow"><label>שם הפעולה</label><input class="label" value="${esc(t.label)}" placeholder="לדוגמה: לבצע תרגיל"></div><div style="width:120px"><label>שעה</label><input class="time" type="time" value="${t.time||'20:00'}"></div></div><label>תיאור או הערה <span class="optional">(אופציונלי)</span></label><textarea class="description" rows="2" placeholder="לדוגמה: 3 סטים של 10 חזרות">${esc(t.description)}</textarea><label>תדירות</label><select class="freq"><option value="daily">כל יום</option><option value="every_other">יום כן / יום לא</option><option value="every_n">כל X ימים</option><option value="weekdays">ימים מסוימים בשבוע</option></select><div class="freqExtra"></div><div class="switchline"><input class="notify" type="checkbox" ${t.notify?'checked':''}><span>התראה בשעה שנבחרה</span></div><button class="btn danger smallbtn del" style="margin-top:10px">הסר פעולה</button>`;
...
function setupNavigation(){if(!history.state||!history.state.app){history.replaceState({app:true,route:'exit',depth:0},'');history.pushState({app:true,route:'main',view:'todayView',depth:1},'');}else if(history.state.route==='exit'){history.pushState({app:true,route:'main',view:'todayView',depth:Number(history.state.depth||0)+1},'');}applyRoute(history.state);}
...
async function importDataFile(file){
  ...
  try{
    ...
    state.version=6;state.settings||={theme:'system'};state.reminders||=[];state.tasks||=[];
...
// events
document.getElementById('customRelativeToggleBtn').onclick=toggleCustomRelative;document.getElementById('customRelativeAmount').onchange=setCustomRelativeReminder;document.getElementById('customRelativeUnit').onchange=setCustomRelativeReminder;document.getElementById('taskDetailBack').onclick=()=>history.back();document.getElementById('openToolsBtn').onclick=()=>document.getElementById('toolsMenu').classList.toggle('hidden');document.getElementById('toolsSettingsBtn').onclick=()=>{document.getElementById('toolsMenu').classList.add('hidden');pushRoute({route:'main',view:'appSettingsView'})};document.getElementById('toolsArchiveBtn').onclick=()=>{document.getElementById('toolsMenu').classList.add('hidden');pushRoute({route:'main',view:'archiveView'})};
